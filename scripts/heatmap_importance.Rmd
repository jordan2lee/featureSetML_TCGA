...
```{r dev one, eval=FALSE}
# # for (c in colnames(df_fts)[2:ncol(df_fts)]){
# #   print(c)
# #   print(sum(df_fts[,c]))
# # }
...
'N:METH', 'N:MIR', 'I:CNVR', 'B:MUTA', 'N:GEXP'
...
test<- scale(t(test), center=TRUE, scale=TRUE) 
...
ftnames_order <- c() # featurename vector
for (i in heatmap_order){
  add_ft <- colnames(test)[i]
  ftnames_order <- c(ftnames_order, add_ft)
}
# 2. Get new matrix that is ordered by heatmap clustering
mat2 <- test[,match(ftnames_order, colnames(test))]
#####
# Build annotation bars of teams feature sets
#####
# 1. df of all teams. match ft order in heatmap
...
# NEW
# If only one feature set ht_rows/cols manually
if (is.null(ht_rows) && is.null(ht_cols)){
  ht_rows <- length(mat2)
  ht_cols <- 1
}
