...
imp_mat <- fread(file_imp_aklimate) %>% as.data.frame()
######
```
...
get_base_heatmap <- function(prefix, cancer, header_jadbio, header_cforest, header_aklimate, header_subscope, header_skgrid){
  ...
  # tiff(
  #   paste(cancer, 'ht_base', unlist(strsplit(prefix, ':'))[2],'.tiff',sep='_'),
  #   width = 800,
  #   height = 800,
  #   res = 200,
  #   compression = "none"
  ...
  image <- anno(
    paste(cancer, 'TEST_ht_base', unlist(strsplit(prefix, ':'))[2],'.pdf',sep='_'),
    full.names=TRUE, 
    10
    )
  anno = anno_image(image)
  draw(fig, adjust_annotation_extension = FALSE)
  # draw(fig, adjust_annotation_extension = FALSE)
  dev.off()
...
if (cancer == 'LGGGBM' & top == c("INTERFERON_GAMMA_RESPONSE", "MTORC1_SIGNALING", "IL2_STAT5_SIGNALING", "INFLAMMATORY_RESPONSE", "ESTROGEN_RESPONSE_LATE") ){
  ...
  s_imp_mat <- imp_mat[,c('features', 'importance')]
  # Min max normalize importance scores
  minmax_vals <- mm_normal(s_imp_mat$importance)
  s_imp_mat <- cbind(s_imp_mat, minmax_vals)
  # Map and order ft importances to pooled ft order
  aklimate_minmax <- c()
  sanity_check_fts <- c()
  aklimate_importance_fts <- s_imp_mat$features
  for (pooled_ft in team_df$featureID){
    ...
    if (pooled_ft %in% aklimate_importance_fts){
      mm <- s_imp_mat[s_imp_mat$'features'==pooled_ft,]$minmax_vals
  ...
  # Set up saving fig packet
  pdf(paste(cancer, 'ht_ftimportance_top5', unlist(strsplit(platform_of_interest, ':'))[2],'.pdf',sep='_'), onefile=FALSE)
  draw(fig, adjust_annotation_extension = FALSE)
  dev.off()
