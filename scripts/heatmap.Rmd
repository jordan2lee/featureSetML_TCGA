...
```{r functions}
get_base_heatmap <- function(prefix, cancer){
...
for (prefix in platforms){
  # Create Base Heatmap
  results_list <- get_base_heatmap(prefix, cancer)
...
# Set up saving fig packet
setwd(outdir)
pdf(paste(cancer, 'hallmark_fts', unlist(strsplit(prefix, ':'))[2],'.pdf',sep='_'), onefile=FALSE)
...
# Set up saving fig packet
setwd(outdir)
pdf(paste(cancer, 'hallmark_hist', unlist(strsplit(prefix, ':'))[2],'.pdf',sep='_'), onefile=FALSE)
...
# Set up saving fig packet
setwd(outdir)
pdf(paste(cancer, 'ht_top5', unlist(strsplit(prefix, ':'))[2],'.pdf',sep='_'), onefile=FALSE)
draw(fig)
dev.off()
```
