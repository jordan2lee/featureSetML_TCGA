...
upsetmode <- 'distinct'
init_vector <- c(clust1 = 0, clust2 = 0, clust3=0) # where len == n_clusters
team_order_upset <- c('JADBIO', 'CloudForest', 'AKLIMATE', 'SUBSCOPE', 'SciKitGrid')

file = '../data/heatmap/BRCA_vals_muta.tsv'
cancer = 'BRCA'
datatype <- 'B:MUTA:'
...
# 1. WSS
res_wss <-fviz_nbclust(df, FUNcluster = hcut, method = "wss", k.max = 6, print.summary = TRUE) #switch to 6 to avoid error
# print(res_wss$labels)
res_wss

# 2. Silhouette
res_sill <-fviz_nbclust(df, FUNcluster = hcut, method = "silhouette", k.max = 6, print.summary = TRUE) #switch to 6 to avoid error
res_sill
...
#######
# 1. kmeans clustering
# fviz_nbclust(df, kmeans, method = "wss", print.summary = TRUE) #err
```
### Dendrogram
